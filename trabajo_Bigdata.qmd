...
1.  **La primera sección** consiste en mostrar la evolución del IPC en España y relacionar los cambios de este con distintos eventos.
...
3.  **Tercera sección donde analizaremos el impacto de sectores específicos**
...
1.  **Crisis Económicas**:
    ...
    -   Posteriormente, la recuperación de la economía española fue gradual. A lo largo de la siguiente década, el IPC se mantuvo a niveles moderados, con algunas fluctuaciones, reflejando el proceso de consolidación económica del país.
        ...
        2.  **Años de Alta Inflación (2007, 2011, 2017)**:
    ...
    3.  **Recuperación y Estabilidad (2015-2020)**:
    ...
    5.  **Crisis Energética Global (2022-2023)**:
...
```
        ...
        # Reemplazar las comas por puntos en la columna 'total' (asumiendo que esta columna existe)
        df <- df %>% mutate(total = stringr::str_replace(total, ",", "."))

        # Seleccionar las columnas necesarias y filtrar los datos por el mes de diciembre (M12)
        df <- df %>%
          select(-c(2)) %>%  # Eliminar la segunda columna (verifica si es necesario)
          filter(grepl("M12", año)) %>%  # Filtrar por mes de diciembre (M12)
          mutate(
            año = substr(año, 1, 4),  # Extraer los primeros 4 caracteres como año
            mes = substr(año, 5, 6)   # Extraer los caracteres 5 y 6 como mes
          )

        # Filtrar los datos para los años entre 1999 y 2008 y procesar la columna 'total'
        df <- df %>%
          filter(año >= 1999 & año <= 2008) %>%  # Filtrar años entre 1999 y 2008
          mutate(
            total = gsub(",", ".", total),  # Reemplazar comas por puntos
            total = as.numeric(total),      # Convertir la columna 'total' a numérico
            año = as.numeric(año)          # Convertir la columna 'año' a numérico
          ) 
        ...
        # Asegúrate de que 'total' se convierte en numérico
        df <- df %>%
          mutate(total = gsub(",", ".", total)) %>%   # Reemplaza las comas por puntos
          mutate(total = as.numeric(total))  # Convierte 'total' a numérico

        # Crear la tabla resumen con los años en las columnas y las CCAA en las filas
        tabla_resumen <- df %>%
          group_by(CCAA, año) %>%
          summarise(Total = sum(total, na.rm = TRUE)) %>%
          pivot_wider(
...
```
        ...
        -   Navarra:
            ...
            -   Presenta la mayor variación del IPC en 2019 con 1.3%.

            -   Esto se debe probablemente a factores como la inflación general en España y la evolución de los precios en sectores clave como la energía, alimentación, vivienda, etc.

        -   País Vasco:

            -   Registra la segunda mayor variación en 2019 con 1.2%.

            -   Al igual que Navarra, esto puede estar relacionado con la dinámica inflacionista a nivel nacional y la evolución de los precios en sectores relevantes para la región.

        -   Castilla y León, Baleares :

            -   Estas comunidades muestran variaciones del IPC por encima del 1% en 2019.

        -   Factores como la estructura productiva, el peso de determinados sectores y la evolución de los precios en los mismos pueden explicar estas mayores variaciones.

        En general, las comunidades con mayor variación suelen ser aquellas con economías más diversificadas y expuestas a la evolución de precios a nivel nacional. Factores como la composición sectorial, la dependencia de determinados bienes y servicios, y la dinámica inflacionista general pueden influir en estas diferencias regionales.

        Analizando la información proporcionada en la tabla, podemos hacer las siguientes observaciones sobre la evolución general del IPC en las comunidades autónomas de España durante el periodo 2019-2023:

        -   Período 2019-2020:

            -   En 2019, todas las comunidades presentaron variaciones positivas del IPC, con valores entre 0.3% y 1.3%.

            -   En 2020, la mayoría de las comunidades registraron variaciones negativas, reflejando una caída generalizada de la inflación debido al impacto de la pandemia de COVID-19. Las caídas más pronunciadas se observaron en Castilla-La Mancha (-0.9%) y la Comunidad de Madrid (-0.8%).

        -   Período 2021-2022:

            -   A partir de 2021, se observa un repunte generalizado de la inflación en todas las comunidades.

            -   Las variaciones del IPC alcanzaron valores entre 5.5% y 7.6% en 2021, evidenciando un fuerte aumento de los precios.

            -   En 2022, las variaciones siguieron siendo elevadas, aunque ligeramente inferiores a 2021, situándose entre 4.9% y 6.8%.

        -   Período 2023:

            -   Para 2023, se aprecia una desaceleración del ritmo de crecimiento del IPC en todas las comunidades.

            -   Las variaciones se sitúan entre 2.4% y 3.8%, mostrando una tendencia a la moderación de la inflación.
